...
int main (int argc, char *argv[]) {
    ...
    for (size = 1; size <= MAX_MSG_SIZE_PT2PT; size = (size ? size * 2 : 1)) {
        ...
        if (0 == mype_node) {
            ...
            CUDA_CHECK(cudaMemset(counter_d, 0, sizeof(unsigned int) * 2));
            // CUDA_CHECK(cudaStreamSynchronize(stream));
            latency_warp_nbi<<<4, 1024>>>(destination, counter_d,size, mype_node, skip);
            cudaEventRecord(start);
            //CUDA_CHECK(cudaEventSynchronize(start));
            // bw2<<<4,1024>>> (destination, counter_d, size, mype_node,loop);
            CUDA_CHECK(cudaMemset(counter_d, 0, sizeof(unsigned int) * 2));
            latency_warp_nbi<<<4, 1024>>>(destination, counter_d,size, mype_node, loop);
            //---------------------
            cudaEventRecord(stop);
            CUDA_CHECK(cudaEventSynchronize(stop));
            //CUDA_CHECK(cudaDeviceSynchronize());
    ...
    for (size = 1; size <= MAX_MSG_SIZE_PT2PT; size = (size ? size * 2 : 1)) {
        ...
        if (0 == mype_node) {
            ...
            CUDA_CHECK(cudaMemset(counter_d, 0, sizeof(unsigned int) * 2));
            // CUDA_CHECK(cudaStreamSynchronize(stream));
            latency_block_nbi<<<4, 1024>>>(destination, counter_d,size, mype_node, skip);
            cudaEventRecord(start);
            CUDA_CHECK(cudaMemset(counter_d, 0, sizeof(unsigned int) * 2));
            //CUDA_CHECK(cudaEventSynchronize(start));
            // bw2<<<4,1024>>> (destination, counter_d, size, mype_node,loop);
            latency_block_nbi<<<4, 1024>>>(destination, counter_d,size, mype_node, loop);
            //---------------------
            cudaEventRecord(stop);
            CUDA_CHECK(cudaEventSynchronize(stop));
            //CUDA_CHECK(cudaDeviceSynchronize());
